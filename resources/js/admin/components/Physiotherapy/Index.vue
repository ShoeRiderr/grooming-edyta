            ...
            <div class="card">
                <ul class="list-group list-group-flush" v-for="physiotherapy in physiotherapy.data" :key="physiotherapy.id">
                    <li class="list-group-item border-bottom">
                        <div class="d-flex">
                            <span>{{physiotherapy.title}}</span>
                            <div class="ml-auto">
                                <router-link
                                    class="btn btn-outline-primary btn-sm mr-2"
                                    :to="{ name: 'admin.physiotherapy.edit', params: { id: service.id } }"
                                >
                                    Edytuj
                                </router-link>
                                <button
                                    class="btn btn-outline-danger btn-sm"
                                    @click.prevent="deletePhysiotherapy(physiotherapy.name, physiotherapy.id)"
                                    :disabled="loading"
                                >
                                    Usuń
                                </button>
                ...
                <pagination class="p-4 mb-0 float-right" :data="physiotherapy" @pagination-change-page="fetchphysiotherapys"></pagination>
            </div>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        deletePhysiotherapy(physiotherapy, id) {
            ...
            if(confirm(`Czy na pewno chcesz usunąć physiotherapy ${physiotherapy}?`)) {
                axios.delete(`/api/admin/physiotherapy/${id}`)
                .then(_ => {
                    this.$notify({
                        type: 'success',
                        title: 'Sukces',
